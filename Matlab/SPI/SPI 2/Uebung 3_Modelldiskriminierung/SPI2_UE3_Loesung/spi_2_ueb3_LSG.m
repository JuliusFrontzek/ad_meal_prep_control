...
%         pUB = [0.5;0.5;1;10;10];         
    ...
    %% Aufgabe: 1c
    p0 = pLB+rand*(pUB-pLB); % zufälliger Parameterwert zwischen obere und untere Schranke
...
aic_distance(1) = AIC_c(1) - AIC_min;
aic_distance(2) = AIC_c(2) - AIC_min;
aic_distance(3) = AIC_c(3) - AIC_min;

aic_sum = exp( (-1)* aic_distance(1) /2) + exp( (-1)* aic_distance(2) / 2) + exp( (-1)* aic_distance(3) / 2);

w_aic(1) = exp( (-1) *aic_distance(1) /2 )  / aic_sum;
w_aic(2) = exp( (-1) *aic_distance(2) /2 )  / aic_sum;
w_aic(3) = exp( (-1) *aic_distance(3) /2 )  / aic_sum;
