%% Version
% (R2022b) Update 5
% Erstelldatum: 04.08.2023
% Autor: Simon Hellmann

% create synthetic measurement data for Kalman Filtering
% scenario: no delays, online + 2 offline rates (Std.-Analyses, Acids)
...
tFeedOn = [1;2.5;4.5;6];  % beginning times of feedings
feedingDurations = [intMed; intShort; intMed; intShort];
tFeedOff = tFeedOn + feedingDurations; % end times of feedings
tEvents = sort([0;tFeedOn;tFeedOff]); 
dt = 0.25/24;           % sample time [h], converted to [d]
...
% case 2: online, and offline measurements at 2 different rates
dtStd = 1;      % sample time for standard measurements [d]
% times when standard samples were taken (TS, VS, NH4-N:
tStdSample = (0:dtStd:tEnd)';  % no delay 

% interpolate xSim at standard offline sample times: 
xSolStd = interp1(tOverall,xSim,tStdSample);
...
for kk = 1:NStd
    fullOutput = g(xSolStd(kk,:)',cNum)';
    yCleanStd(kk,:) = fullOutput(qOn+1:qOn+qStd); 
end
...
sigmasStd = [sigmaSIN, sigmaTS, sigmaVS]; % standard measurements' sigmas
...
scatter(tStdSample,yMeasStd(:,1),'DisplayName','noisy',...
        'Marker','x', 'Color', colorPaletteHex(1), 'LineWidth',1.5); 
hold on; 
...
save('Messung_ADM1_R4_fracIntensiveSampling.mat', 'MESS', 'params')
