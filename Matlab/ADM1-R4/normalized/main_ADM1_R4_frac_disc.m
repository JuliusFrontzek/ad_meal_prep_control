%% Version
% (R2022b) Update 5
% Erstelldatum: 2.6.2023
% Autor: Simon Hellmann

% create the MESS-vector (synthetic measurement data) for Kalman Filtering
% through discrete-time integration with explicite Euler
...
dt = 0.001/24;              % sample time [h], converte to [d]
...
nSample = numel(tGrid);   % number of sampling points in discrete time grid
...
syms tS real positive    % sampling time

% continuous-time versions of right-hand side (symbolic)
dynamics = BMR4_AB_frac_ode_sym(xS, uS, xiS, thS, cS, aS); % symbolic object
outputs = BMR4_AB_frac_mgl_sym(xS,cS); 

% discrete-time counterparts (explicit Euler): 
dynamicsDisc = xS + tS*dynamics;
...
xPlus1 = matlabFunction(dynamicsDisc, 'Vars', {xS, uS, xiS, thS, cS, aS, tS}); 
...
%% Solve continuous-time ODE via iterative solution of constant feeding regimes (on or off)
...
%% solve discrete-time ODE with fixed time grid: 
% integrate over tGrid
xSolDisc = nan(nSample,nStates);    % allocate memory

% interpolation der Fütterungen: 
inputMatDisc = interp1(tEvents, inputMat, tGrid, 'previous', 'extrap'); 

xSolDisc(1,:) = x0Init';  % start in same initial value as continuous integration
xOld = x0Init;          % to be overwritten

tic
for k = 2:nSample
    % fixed grid of feed vol flows and inlet concentrations:
    feedVolFlowCurr = inputMatDisc(k-1,1); 
    xInCurr = inputMatDisc(k-1,2:end)'; 

    % discrete-time state update equation (explicit Euler):
    xNext = xPlus1(xOld,feedVolFlowCurr,xInCurr,thNum,cNum,aNum,dt); 
    
    % clipping if states become negative or out of reasonable bounds:
    xMax = 50; 

    if any(xNext > xMax)
        xNext(xNext > xMax) = xMax/10; 
    elseif any(xNext < 0)
        xNext(xNext < 0) = 0; 
    end
...
%         xNext()
    ...
    % save and update state for next iteration: 
    xSolDisc(k,:) = xNext'; 
    xOld = xNext; 
...
xSol = xSim(idxGrid,:);
q = 6;   
yCleanCont = zeros(nSample,q); % allocate memory
yCleanDisc = zeros(nSample,q); % allocate memory
for k = 1:nSample
    yCleanCont(k,:) = g(xSol(k,:)',cNum)'; % output of continuous implementation
    yCleanDisc(k,:) = g(xSolDisc(k,:)',cNum)'; % output of discrete implementation
end
...
hold on; 
plot(tGrid,yCleanCont(:,1)/24,'DisplayName','clean cont',...
     'LineStyle','-.', 'Color', colorPaletteHex(2), 'LineWidth',1.5); 
plot(tGrid,yCleanDisc(:,1)/24,'DisplayName','clean disc',...
...
scatter(tGrid,yMeas(:,2),'DisplayName','noisy',...
        'Marker','.', 'Color', colorPaletteHex(1), 'LineWidth',1.5);
hold on
plot(tGrid,yCleanCont(:,2),'DisplayName','clean cont',...
     'LineStyle','-.', 'Color', colorPaletteHex(2), 'LineWidth',1.5);
plot(tGrid,yCleanDisc(:,2),'DisplayName','clean disc',...
     'LineStyle','-.', 'Color', 'k', 'LineWidth',0.5); 
...
scatter(tGrid,yMeas(:,3),'DisplayName','noisy',...
        'Marker','.', 'Color', colorPaletteHex(1), 'LineWidth',1.5); 
hold on; 
plot(tGrid,yCleanCont(:,3),'DisplayName','clean',...
     'LineStyle','-.', 'Color', colorPaletteHex(2), 'LineWidth',1.5);
plot(tGrid,yCleanDisc(:,3),'DisplayName','clean disc',...
     'LineStyle','-.', 'Color', 'k', 'LineWidth',0.5); 
...
scatter(tGrid,yMeas(:,4),'DisplayName','noisy',...
        'Marker','.', 'Color', colorPaletteHex(1), 'LineWidth',1.5); 
hold on; 
plot(tGrid,yCleanCont(:,4),'DisplayName','clean',...
     'LineStyle','-.', 'Color', colorPaletteHex(2), 'LineWidth',1.5);
plot(tGrid,yCleanDisc(:,4),'DisplayName','clean disc',...
     'LineStyle','-.', 'Color', 'k', 'LineWidth',0.5); 
...
scatter(tGrid,yMeas(:,5),'DisplayName','noisy',...
        'Marker','.', 'Color', colorPaletteHex(1), 'LineWidth',1.5); 
hold on; 
plot(tGrid,yCleanCont(:,5),'DisplayName','clean cont',...
     'LineStyle','-.', 'Color', colorPaletteHex(2), 'LineWidth',1.5);
plot(tGrid,yCleanDisc(:,5),'DisplayName','clean disc',...
...
scatter(tGrid,yMeas(:,6),'DisplayName','noisy',...
        'Marker','.', 'Color', colorPaletteHex(1), 'LineWidth',1.5)
hold on; 
plot(tGrid,yCleanCont(:,6),'DisplayName','clean',...
     'LineStyle','-.', 'Color', colorPaletteHex(2), 'LineWidth',1.5);
...
MESS.yClean = yCleanCont;  
MESS.yMeas = yMeas; 
MESS.C = noiseCovMat; % accurate values from sensor data sheets

save('Messung_ADM1_R4_frac_disc.mat', 'MESS', 'params')
