...
% Solve ODE of mass-based ADM1-reductions:
switch flagModel
    case 3
        model = 'ADM1_R3'; 
        modelParameters = parameters.ADM1_R3.Variables; 
        ...
        systemInputSS = systemInput(2,:);   % only take the second feeding, which perturbes the system out of its steady state
        systemInputSS(1) = 0;   % set initial feeding time to zero
        odeFun = @(t,x) ADM1_R3_mass_edit(t,x,systemParameters,systemInputSS,modelParameters); 
        x0 = initial.ADM1_R3.Variables; 
    case 4
        model = 'ADM1_R4'; 
        modelParameters = parameters.ADM1_R4.Variables; 
        systemInput = input.ADM1_R4.Variables;
        systemInputSS = systemInput(2,:);   % only take the second feeding, which perturbes the system out of its steady state
        systemInputSS(1) = 0;   % set initial feeding time to zero
        odeFun = @(t,x) ADM1_R4_mass_edit(t,x,systemParameters,systemInputSS,modelParameters); 
        x0 = initial.ADM1_R4.Variables;
end

time_range = [0 300];           % time range in days
% simulate system and save results as struct: 
odeObj = ode15s(odeFun,time_range,x0);
...
switch flagModel
    ...
    case 4
        for i = 1:nSteps
            [t(i),y(i,:)] = ADM1_R4_mass_output(odeObj.x(1,i),odeObj.y(:,i),systemParameters,modelParameters);
        end
end
...
%         Snh4SS = xSS(4) - xSS(15);
        ...
        % re-order outputs: 
        ySS = [volFlow, pch4, pco2, SIN, TS, nan]'; % leave VS nan for now
...
fileName = ['SteadyState_',num2str(model),'_Soeren.mat']; 
