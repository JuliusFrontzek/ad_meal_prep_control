...
load('generatedOutput\Messung_ADM1_R3_frac_norm.mat')
...
RNorm(8,8) = RNorm(8,8)*1E2;    % decrease faith in S_ac measurements
...
for kk = 1:q
    measurementsNorm = MESS.yMeas(:,kk)/TyNum(kk); 
    
    % ignore the first value because that's only the output of x0:
    estimatedMeasurementsNorm = EKFOutputNorm(2:end,kk); 
    
    % get RMSSEs of EKF and measurements (normalized): 
    [RMSSENorm(kk)] = computeRMSSE(measurementsNorm,estimatedMeasurementsNorm); 
end
