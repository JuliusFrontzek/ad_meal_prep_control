% create the MESS-vector using only models acc. to arXiv paper
...
odeFunSS = @(t,x) BMR4_AB_ode(t,x,inputVectorSS,AC); 
...
for cI = 1:nIntervals
    ...
    modelOdeFun = @(t, x) BMR4_AB_ode(t,x,inputVector,AC);
...
xSol = xSim(idxGrid,:);
yClean = BMR4_AB_mgl_mat(xSol,AC.c);
...
save('SimonsMessung_ADM1_R4.mat', 'MESS', 'AC', 'sigmas')
