...
function [xPlusNorm,PPlusNorm] = constrUnscKalmanFilterKolasAugmentedNorm(xOldNorm,POldNorm, ...
...
% augment x and P: 
xOldAugNorm = [xOldNorm;zeros(nStates,1)]; 
POldAugNorm = blkdiag(POldNorm,QNorm);  % (2*nStates, 2*nStates)
...
PPlusTempNorm = PPlusFullyAugmentedCaseNorm + KNorm*RNorm*KNorm'; % different formula for augmented process noise case (normalized)!
