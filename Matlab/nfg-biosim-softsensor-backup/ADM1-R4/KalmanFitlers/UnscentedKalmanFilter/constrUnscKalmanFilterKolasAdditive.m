...
function [xPlus,PPlus] = constrUnscKalmanFilterKolasAdditive(xOld,POld, ...
...
for k = 1:nSigmaPoints
    ukfCostFun = @(sigmaX) evaluateCUKFCostFun(sigmaX,sigmaXProp(:,k), ...
                                yMeas',R,PMinus,g,params.c); 
    % choose the old sigmaXProp as initial value for optimization:
    sigmaXOpt(:,k) = fmincon(ukfCostFun,sigmaXProp(:,k),A,b); 
end 
