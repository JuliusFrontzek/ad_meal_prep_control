...
% Modell: ADM1-R4 (including ash) using m³ and kg as units, and with
...
kch = 0.25;        % war vorher der Wert für kch    % YZ
kpr = 0.2;          % YZ 
kli = 0.1;          % YZ
kdec = 0.02;        % YZ
...
Kch4 = 0.0011; % [mol/l/bar]  % YZ
Kco2 = 0.025;  % [mol/l/bar]  % YZ
...
kp = 5e4;       % friction parameter [m³/bar/d]. Achtung: eigentlich ist 
% die Einheit [L/bar/d], aber bei Umrechnung verzieht sich die Gasproduktion/-Zusammensetzung!
% Da niemand genau weiß, woher der Parameter kommt, lasse den Zahlenwert
% trotz sonstiger Umrechnung auf m³ gleich!
...
Vg = 0.1*Vl;    % maintain 10% ratio
rho = 1e3;      % mass density of digestate. unit change [kg/l] -> [kg/m³] 
Mch4 = 16;   % molar mass CH4 [g/mol] but with correction to get p in bar % YZ
Mco2 = 44;   % molar mass CO2 [g/mol] but with correction to get p in bar % YZ
...
xIn = [0,     0,   0.592, 960.512, 23.398, 4.75,  1.381, 0,    17,     0,    0]'; % [g/l], % YZ
% XY: finde hier raus, welche Eingangs-Konzentrationen das wirklich sind
...
% feedMax = 195.6;    % m³/d, upscaling with factor 1630 like Vl
...
outputs = BMR4_AB_mgl_bac_sym(xS,cS); 
...
sigmaV = 0.2*24*1e-3; % erhöhe künstlich das Messrauschen (10x); Trommelgaszähler Labor [L/h] --> [m³/d]
sigmaCh4 = 20*0.2/100; % [Vol-%] -> [-]; erhöhe künstlich das Messrauschen (20x); ideale Gase und p0 ungefähr 1 bar: -> [bar]
sigmaCo2 = 0.2/100; % [Vol-%] -> [-]; erhöhe künstlich das Messrauschen; ideale Gase und p0 ungefähr 1 bar: -> [bar]
sigmaSIN = 0.12;    % NH4-N [kg/m³] = [g/L]
sigmaTS = 1.73/100; % Trockenschrank großer Tiegel [%] -> [-]
sigmaVS = 0.31/100; % [%] -> [-]
sigmaBac = 0.5;     % microbial biomass [kg/m³] = [g/L]

% combine all in sigma matrix and covariance matrix:
sigmas = [sigmaV, sigmaCh4, sigmaCo2, sigmaSIN, sigmaTS, sigmaVS, sigmaBac]; 
...
fileName = 'Messung_ADM1_R4_bac_m3.mat'; 
