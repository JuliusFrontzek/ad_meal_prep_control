...
% Modell: ADM1-R4 (mit Asche, nur 1 CH-Fraktion), Konzentrationen in mg/m³
% und Volumina in m³
...
K_H_ch4 = 1e-2*0.0011; % unit change [mol/l/bar] -> [mol/J]
K_H_co2 = 1e-2*0.025;  % unit change [mol/l/bar] -> [mol/J]

% miscellaneous parameters (Weinrich, 2017, Tab. B.7): 
R = 8.314;         % id. gas constant [J/mol/K]
...
k_p = 5e1;       % friction parameter [m³/bar/d]

T_op    = 273.15;   % operating temperature [K]
V_liq = 100e-3;     % liquid volume, aus Sörens GitHub. unit change [l] -> [m³]
V_gas = 10e-3;      % gas volume, aus Sörens GitHub.  unit change [l] -> [m³]
rho_liq = 1e3;      % mass density of digestate. unit change [kg/l] -> [kg/m³] 
M_ch4 = 16e2;       % molar mass CH4 [g/mol] but with correction to get p in bar
M_co2 = 44e2;       % molar mass CO2 [g/mol] but with correction to get p in bar
...
fileName = 'Messung_ADM1_R4_m3_norm.mat'; 
