%% Version
% (R2022b) Update 5
% Erstelldatum: 11.09.2023
% Autor: Simon Hellmann

% draw measurement data from Intentiv-Beprobung and transform this into
% measurement struct MESS
    
clc
clear
close all

%% pull relevant measurement data, store it as tables in struct (one for each sheet): 

addpath('C:\Users\shellmann\Desktop')
myTableName = 'ADM1_Intensivbeprobung_23_R4.35_short.xlsx';
mySheets = {'data','blueVaryData','input','liquid_data'};    % choose between 'input' and 'data'
nSheets = numel(mySheets); 
endDay = datetime(2022,05,06);  % we want only data before this date
T = struct;     % allocate memory
for k = 1:nSheets

    opts = detectImportOptions(myTableName,'Sheet',mySheets{k},'NumHeaderLines',0);
    opts.VariableNamesRange = 'A1'; % start searching for variable names here
    opts.DataRange = 'A3';          % start searching for data here

    % make sure offline data are imported as doubles (instead of strings):
    if strcmp(mySheets{k},'liquid_data') == 1
        disp('durchsuche liquid data...')
        opts = setvartype(opts,{'VOA','NH4_N','TS', 'VS'},'double');
        disp('Habe Variablentypen der offline-Messwerte angepasst!')
    end
    % 

    % read full table:
    TPre = readtable(myTableName,opts,'Sheet',mySheets{k},'ReadVariableNames',true);
    
    % extract and save only  data up to certain end date: 
%     timeArray = TPre{:,1};
    idxTime = TPre{:,1} < endDay; 
    T.(mySheets{k}) = TPre(idxTime,:);
end 

nFields = numel(fieldnames(T)); 

%% gather time stamps and measurement data: 

% 1. BlueVaryData (online):
startTimeBlueVary = T.blueVaryData{1,1}; 
dateTimeBlueVary = T.blueVaryData{:,1}; 
% compute absolute times and convert to hours: 
absDateTimesBlueVary = dateTimeBlueVary - startTimeBlueVary; 
absHoursBlueVary = hours(absDateTimesBlueVary); 

% compute gas volume flow [L/h] from diffences in gas volume: 
gasVolumes = T.blueVaryData{:,'Biogas'}; 
deltaVolumes = diff(gasVolumes); 
deltaHours = diff(absHoursBlueVary); 
gasVolumeFlows = deltaVolumes./deltaHours;  % [L/h]

% correct time vector starting from 2nd value because computation of 
% of gas vol flow killed the first time stamp:
startTimeBlueVaryEff = T.blueVaryData{2,1}; 
dateTimeBlueVaryEff = T.blueVaryData{2:end,1}; 

% drop first value of CH4 and CO2: 
CH4 = T.blueVaryData{2:end,"Methane"};
CO2 = T.blueVaryData{2:end,"CO2"}; 

% 2. liqid data (pH online, others offline)
% pull pH measurements: 
startTimeLiquidData = T.liquid_data{1,1}; 
dateTimeLiquidData = T.liquid_data{:,1}; 
pH = T.liquid_data{:,"pH_Logger"}; 
% pull remaining offline measurements: 
TS = T.liquid_data{:,"TS"}; 
VS = T.liquid_data{:,"VS"}; 
NH4 = T.liquid_data{:,"NH4_N"}; 
VOA = T.liquid_data{:,"VOA"}; 

%% gather all feedings with corr. time stamps: 

feedVolumes = T.input{:,'Volume'};  % [L]
feedingDurations = duration(T.input{:,'dt_Feed'});    % convert strings to durations
feedingDurationsHours = hours(feedingDurations); % [h]
feedVolFlows = feedVolumes./feedingDurationsHours;       % [L/h]

dateTimeFeedingsStart = T.input{:,1}; 
dateTimeFeedingsEnd = dateTimeFeedingsStart + feedingDurations;
startTimeFeedings = T.input{1,1};

%% compute unified relative times of all measurements and feedings [hours]:
startTimes = [startTimeBlueVaryEff; startTimeLiquidData; startTimeFeedings]; 
startTimeOverall = min(startTimes); 
absDateTimesBlueVaryEff = dateTimeBlueVaryEff - startTimeOverall; 
absDateTimesLiquid = dateTimeLiquidData - startTimeOverall; 
absDateTimesFeedingsStart = dateTimeFeedingsStart - startTimeOverall; 
absDateTimesFeedingsEnd = dateTimeFeedingsEnd - startTimeOverall; 
% convert into hours: 
absHoursBlueVaryEff = hours(absDateTimesBlueVaryEff); 
absHoursLiquid = hours(absDateTimesLiquid); 
absHoursFeedingsStart = hours(absDateTimesFeedingsStart); 
absHoursFeedingsEnd = hours(absDateTimesFeedingsEnd); 

% XY: vereine alle Zeitschriebe (online/offline/Fütterung)
tMinor = unique([absHoursFeedingsStart;absHoursFeedingsEnd;absHoursBlueVaryEff;absHoursLiquid]); % [h]
nSamplesMinor = numel(tMinor); 

MEASUnite = nan(nSamplesMinor,9);   % allocate memory
MEASUnite(:,1) = tMinor;            % insert times as first column
% determine correct row positioning of diff. data types in MEASUnite (right
% timing):
[~,idxBlueVary] = ismember(absHoursBlueVaryEff,tMinor);       % blueVary data
[~,idxLiquid] = ismember(absHoursLiquid,tMinor);                % liquid data
[~,idxFeedStart] = ismember(absHoursFeedingsStart,tMinor);   % feed starts
[~,idxFeedEnd] = ismember(absHoursFeedingsEnd,tMinor);         % feed ends

% insert data: 
MEASUnite(idxBlueVary,2:4) = [gasVolumeFlows,CH4,CO2];  % blueVary data
MEASUnite(idxLiquid,[5,7,8]) = [pH,TS,VS];              % liquid data
% XY Achtung: leave NaNs for S_IN and S_ac cause currently no measurements
% available 

bla = 1; 

%% XY create inputMat with feeding times


%% create unified MESS-Array from measurements with NaNs where no (offline) measurement was returned 
