...
% absolute value!
a = [0.2482,  0.6809,   0.0207,     0.0456,    -1,      0,      0,      0,       0.1372,    0,      0,          0; 
...
a([4,10],:) = a([4,10],:)./1000; 
% same for inlet concentrations of water/ash: 
...
th = [kchF, kchS, kpr, kli, kdec, fracChFast]; % [kchF, kchS, kpr, kli, kdec, fracChFast] 
...
odeFunSS = @(t,x) BMR4_AB_frac_ode(t,x,inputVectorSS,AC); 
...
x0SS = x0Soeren; 
x0SS([4,10]) = x0SS([4,10])./1000;  % unit change for h2o and ash
[tVecSS,xSS] = ode15s(odeFunSS,tSpanSS,x0SS); 

% % double-check if steady-state is correct; use Sörens implementation as a
% % validation: 
% inputVectorSSSoeren = [totalFeed/tEnd,xInSoeren]; 
% odeFunSSSoeren = @(t,x) BMR4_AB_frac_ode_soeren(t, x, physParams, inputVectorSSSoeren, modParams);    
% [tVecSSSoeren,xSSSoeren] = ode15s(odeFunSSSoeren,tSpanSS,x0Soeren); 
% % retransform units of h2o and ash for direct comparison with xSS: 
% xSSSoeren(:,[4,10]) = xSSSoeren(:,[4,10])/1000;
% 
% figure()
% plot(tVecSS(1:90),xSS(1:90,[3,4])); 
% hold on
% plot(tVecSSSoeren(1:90),xSSSoeren(1:90,[3,4]),'LineStyle','-.','LineWidth',1.5)
% % legend('H2O arXiv', 'H2O Sören')
% legend('IN arXiv', 'H2O arXiv', 'IN Sören', 'H2O arXiv')
...
for cI = 1:nIntervals
    ...
    modelOdeFun = @(t, x) BMR4_AB_frac_ode(t,x,inputVector,AC);
...
xSol = xSim(idxGrid,:);
nSamples = size(xSol,1); 
nSignals = 6;   
yClean = zeros(nSamples,nSignals); % allocate memory
for k = 1:nSamples
    yClean(k,:) = BMR4_AB_frac_mgl(xSol(k,:),c)'; % Simons Implementierung (arXiv)
end

yCleanSoeren = biogasmodell_mgl(xSol',pFix)';             % Sörens Implementierung
...
save('MessungSimple_ADM1_R4.mat', 'MESS', 'pFix', 'AC')
