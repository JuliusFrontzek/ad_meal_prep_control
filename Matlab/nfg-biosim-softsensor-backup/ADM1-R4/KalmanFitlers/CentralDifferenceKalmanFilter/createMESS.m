% create the MESS-vector for Kalman Filtering
...
tGrid = (0:1/24:tEnd)';             % time grid every 0.5h [d]. The model outputs will be evaluated here later on
...
feedSS = sum(durationFeed.*portions);  
...
% pch4: 
subplot(2,2,1)
plot(tGrid,simDigestionResults(2,:),'r-',...
     'LineWidth',1.5,'DisplayName','BMR4-AB-mgl')
hold on; 
plot(tGrid,yClean(:,1),'b--','DisplayName','biogasmodell_{mgl}')
...
% pco2:
subplot(2,2,2)
plot(tGrid,simDigestionResults(3,:),'r-',...
     'LineWidth',1.5,'DisplayName','BMR4-AB-mgl')
hold on; 
plot(tGrid,yClean(:,2),'b--','DisplayName','biogasmodell_[mgl}')
ylabel('p_{co2} in bar')
legend('Location','SouthEast'); 

% gas volume flow: 
subplot(2,2,3)
plot(tGrid,simDigestionResults(1,:),'r-',...
     'LineWidth',1.5,'DisplayName','BMR4-AB-mgl')
hold on; 
plot(tGrid,yClean(:,3),'b--','DisplayName','biogasmodell_{mgl}')
...
% SIN:  
subplot(2,2,4)
plot(tGrid,simDigestionResults(4,:),'r-',...
     'LineWidth',1.5,'DisplayName','BMR4-AB-mgl')
hold on; 
plot(tGrid,yClean(:,4),'b--','DisplayName','biogasmodell_[mgl}')
