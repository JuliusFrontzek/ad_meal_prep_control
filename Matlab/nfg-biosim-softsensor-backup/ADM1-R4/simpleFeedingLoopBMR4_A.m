...
% xIn for the BMR4+A-Simon (neglect gas solubility, compute biogas production
% in standard way from partial pressures)
...
%      S_IN,  S_h2o,  X_ch,   X_pr, X_li,  X_bac,   X_ash,  S_ch4,g, S_co2,g
xIn = [0.592, 960.512,23.398, 4.75, 1.381, 0,       10,     0,       0]'; % [g/l], xAshIn = 10 selbst gewählt
% xInMaize = [0.764, 663,    314.5,  24.2, 18.1, 0,      10,     0,      0]';    % [g/l] 
...
nIntervals = length(tEvent);    % because we start in steady state and not with zero feeding
[~,idxFeedOn] = ismember(tFeedOn,tEvent); 
feedSS = 4*24;    % steady state feed volume flow [l/h]
feedMax = 60*24;  % max. feed volume flow [l/h]
feedSmall = feedMax/2; % add a smaller feeding at the end of the simulation
portions = [feedSS, feedMax, feedMax, feedMax, feedSmall]; 
...
% construct inputMat and in inputVector: 
inputMat = [tEvent,feedVolFlow,xInMat];
inputVectorSS = [feedSS;xIn];  

% define function handle to determine the steady state:
odeFunSS = @(t,x) BMR4_A_ode(t, x, physParams, inputVectorSS, modParams);    

% determine steady state as initial value for simulation: 
tSpanSS = [0,tSS]; 
x0Soeren = [0.944,956.97,3.26,0.956,0.413,2.569,1,0.315,0.78]';  % Sörens GitHub, xAsh0 selbst gewählt, Sch4_0 und SIC_0 entfernt
[tVecSS,xSS] = ode15s(odeFunSS,tSpanSS,x0Soeren); 

x0 = xSS(end,:)'; 

% % reuse steady state value of BMR4+AB as inital value for simulation
% load('x0_BMR4_AB.mat')
% x0 = x0(3:end);     % extract only relevant parts (no gas solubility for ch4 and co2)
...
for cI = 1:nIntervals
    ...
    modelOdeFun = @(t, x) BMR4_A_ode(t, x, physParams, inputVector, modParams);
...
figure;
plot(tGrid, simDigestionResults(:,end)/24, 'DisplayName', 'q_{gas} (l/h) BMR4+A');
hold on; grid on;
plot(tGrid, simDigestionResults(:,3), 'DisplayName', 'X_{ch} (g/l) BMR4+A');
